...
class TRegExp
{
 ...
 public:
#ifdef ASTR
  TRegExp(const AnsiString Exp);
  bool Match(const AnsiString Str);
  AnsiString Replace(const AnsiString Rpls);
  AnsiString Replace(const char *Rpls);
  AnsiString TRegExp::Replace(int no);
  bool Replace(const AnsiString Rpls, AnsiString &Res);
  bool Replace(const char *Rpls, AnsiString &Res);
  AnsiString RegErrorStr(void);
  void RegErrorStr(AnsiString &Res);
#endif /* ASTR */
  ...
  int pos(int num) {return ((num < NSUBEXP)?(((char *)pregexp->startp[num]-regbol)):-1);}
  int Len(int num) {return ((num < NSUBEXP)?(pregexp->endp[num] - pregexp->startp[num]):-1);}
