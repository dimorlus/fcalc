...
//---------------------------------------------------------------------------
#ifdef ASTR
TRegExp::TRegExp(const AnsiString Exp)
{
 pregexp = NULL;
 regerror = 0;
 pregexp = regcomp(Exp.c_str());
}
#endif /*ASTR*/
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
#ifdef ASTR
bool TRegExp::Match(const AnsiString Str)
{
 return regexec(pregexp, Str.c_str());
}
#endif /*ASTR*/
//---------------------------------------------------------------------------
bool TRegExp::Match(const char *Str)
{
 return regexec(pregexp, Str);
}
...
int TRegExp::Replace(int no, char *Res, int ResSize)
{
 ...
 if (pregexp && (pregexp->startp[no]) && (pregexp->endp[no]))
  {
   int size = pregexp->endp[no] - pregexp->startp[no];
   if (size < ResSize)
    {
...
//---------------------------------------------------------------------------
AnsiString TRegExp::Replace(int no)
{
 char str[8192];
 ...
 if (pregexp && (pregexp->startp[no]) && (pregexp->endp[no]))
  {
   memmove(&str[0], pregexp->startp[no], pregexp->endp[no] - pregexp->startp[no]);
   int j = pregexp->endp[no] - pregexp->startp[no];
   str[j] = '\0';
   if (str[0]) return AnsiString().sprintf("%s", str);
   else return "";
  }
 else return "";
}
...
//---------------------------------------------------------------------------
AnsiString TRegExp::Replace(const AnsiString Rpls)
{
 char str[8192];
 if (regsub(pregexp, Rpls.c_str(), str) && str[0])
  return AnsiString().sprintf("%s", str);
 else return "";
}

//---------------------------------------------------------------------------
AnsiString TRegExp::Replace(const char *Rpls)
{
 char str[8192];
 if (regsub(pregexp, Rpls, str) && str[0])
  return AnsiString().sprintf("%s", str);
 else return "";
}

//---------------------------------------------------------------------------
bool TRegExp::Replace(const char *Rpls, AnsiString &Res)
{
 char str[8192];
 bool res = regsub(pregexp, Rpls, str);
 if (res) Res = str; //Res.sprintf("%s", str);
 return res;
}

//---------------------------------------------------------------------------
bool TRegExp::Replace(const AnsiString Rpls, AnsiString &Res)
{
 char str[8192];
 bool res = regsub(pregexp, Rpls.c_str(), str);
 if (res) Res = str; //Res.sprintf("%s", str);
 return res;
}
#endif /*ASTR*/
...
{
 return regsub(pregexp, Rpls, Res);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
#ifdef ASTR
AnsiString TRegExp::RegErrorStr(void)
{
 return AnsiString().sprintf("%s", serrors[regerror]);
}

void TRegExp::RegErrorStr(AnsiString &Res)
{
 Res = serrors[regerror];
}
#endif /*ASTR*/
...
void  TRegExp::regtail(char *p, char *val)
{
  ...
  if (OP(scan) == BACK) offset = scan - val;
   ...
   while (replace[i-1])
    {
      ...
      if ((prog->startp[no]) && (prog->endp[no]))
       {
        int size = prog->endp[no] - prog->startp[no];
        if (j+size < destsize)
         {
